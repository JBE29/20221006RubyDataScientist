...
def Q1(handle)
    ...
    print "Il y a "
    print handle.length
    puts " handle Twitter dans l'array."
    end

#done
def Q2(handle)
    # Quel est le handle le plus court ?
    puts ""
    print "Le handle le plus court est "
    puts handle.min_by(&:length)
    end

#done
def Q3(handle)
    # Combien de handle = 5 caractères
    puts ""
    print "Il y a "
    print handle.count {|x| x.length == 6}
    puts " avec 5 caractères. Les voici :"
    puts handle.select {|x| x.length == 6 }
    end

#done    
def Q4(handle)
    # Combien commencent avec un lettre majuscule ? #upercase
    print "Il y a "
    print handle.count{|handle| handle[1].match?(/[[:upper:]]/)}
    puts " handle qui commencent par une majuscule."
    end
#done
def Q5(handle)
    # Trier par ordre alphabéique
    
    #puts handle.each { |el| puts el.handle.to_s }
    puts handle.sort_by(&:downcase)
    end
#done
def Q6(handle)
    # Trier par taille de handle (croissant)
    puts handle.sort_by(&:length)
    end
#done
def Q7(handle)
    # Quel est la position dans l'array de @epenser
    puts handle.index {|element| element == "@epenser"}
    end
...
def Q8(handle)
    ...
    # (nombre de handle avec 1 caractère, nombre de handle avec 2 caractères, nombre de handle avec 3 caractères, etc)
        for i in (1..35)
            if handle.count {|x| x.length == i } != 0
        print handle.count {|x| x.length == i }
        puts " avec #{i} caractères."
            end
        end  
end
